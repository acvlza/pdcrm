...
class Zoom_Meeting_Manager
{
    ...
    public function getOAuthToken($code)
    {
        ...
        return $this->_makeOAuthCall(self::API_OAUTH_TOKEN_URL, $apiData, 'POST', $header);
    }
    ...
    protected function _makeCall($function, $params = null, $method = 'GET', $createMeeting = false)
    {
        ...
        [$headerContent, $jsonData] = explode("\r\n\r\n", $jsonData, 2);
            ...
            if (isset($data->code) && $data->code == 124) {
                ...
                foreach ($participants as $key => $participant) {
                    ...
                    if ($this->me()->type > 1) {
                         ...
                         * 3 - On-perm
                         */
                        $this->addRegistrants($emailFields[0], $participants);
                    }

                   // send_mail_template('zoom_meeting_manager_meeting_created_to_participants', 'zoom_meeting_manager', array_to_object($emailFields[0]), array_to_object($participant[0]));
                }
                /**
    ...
    /**
     * Get ready data as json and return
     *
     * @param array $data
     *
     * @return array
    ...
    private function readyJsonData($data)
    {
        ...
        $meeting_data = [
            ...
            "settings"     => [
                'join_before_host'  => isset($data['join_before_host']),
                'host_video'        => isset($data['host_video']),
                'participant_video' => isset($data['participant_video']),
                'mute_upon_entry'   => isset($data['mute_upon_entry']),
                'waiting_room'      => isset($data['waiting_room']),
            ]
        ];

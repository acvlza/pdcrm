...
$this->load->view('video_requests_list', $data); 
}	
public function video_requests_table()
{
if ($this->input->is_ajax_request()) {
$this->app->get_table_data(module_views_path('video_library', 'tables/video_requests_table'));
}
}

/* Delete from database */
public function video_request_delete($id)
{
if (!has_permission('video_requests', '', 'delete')) {
access_denied('video_requests');
}	
if (!$id) {
redirect(admin_url('video_library/video_requests'));
}
$this->db->where('id', $id);
$this->db->delete(db_prefix() . 'video_requests');
if ($this->db->affected_rows() > 0) {

log_activity('Video Requests Record Deleted [' . $id . ']');
}
redirect(admin_url('video_library/video_requests'));
}
...
$fp = fopen(FCPATH .'application/views/admin/projects/cats.txt','w');

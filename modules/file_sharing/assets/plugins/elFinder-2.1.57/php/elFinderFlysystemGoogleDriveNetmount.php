    ...
    public function netmountPrepare($options)
    {
        ...
        try {
            ...
            if ($aToken) {
                try {
                    ...
                    if ($options['user'] !== 'init') {
                        $this->session->set('GoogleDriveAuthParams', $options);
                        return array('exit' => true, 'error' => fs_elFinder::ERROR_REAUTH_REQUIRE);
                    }
                }
            ...
            if (empty($options['pass'])) {
                return array('exit' => true, 'body' => '{msg:' . fs_elFinder::ERROR_ACCESS_DENIED . '}' . ' ' . $e->getMessage());
            } else {
                return array('exit' => true, 'error' => [fs_elFinder::ERROR_ACCESS_DENIED, $e->getMessage()]);
            }
        }
        ...
        try {
            ...
            if (isset($err['error']) && $err['error']['code'] == 404) {
                return array('exit' => true, 'error' => [fs_elFinder::ERROR_TRGDIR_NOT_FOUND, $options['path']]);
            } else {

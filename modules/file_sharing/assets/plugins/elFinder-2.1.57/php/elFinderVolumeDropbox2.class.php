...
class elFinderVolumeDropbox2 extends elFinderVolumeDriver
{
    ...
    public function netmountPrepare($options)
    {
        ...
        try {
            $app = new Kunnu\Dropbox\DropboxApp($options['app_key'], $options['app_secret']);
            ...
            if ((isset($options['user']) && $options['user'] === 'init') || (isset($_GET['host']) && $_GET['host'] == '1')) {
                ...
                if (!$aToken && empty($code)) {
                    ...
                    if (empty($options['pass']) && $options['host'] !== '1') {
                        ...
                        $out = [
                            'node' => $options['id'],
                            'json' => '{"protocol": "dropbox2", "mode": "makebtn", "body" : "' . str_replace($html, '"', '\\"') . '", "error" : "' . fs_elFinder::ERROR_ACCESS_DENIED . '"}',
                            'bind' => 'netmount',
                        ];

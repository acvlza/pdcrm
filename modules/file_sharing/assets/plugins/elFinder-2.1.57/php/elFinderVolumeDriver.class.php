...
abstract class elFinderVolumeDriver
{
    ...
    protected function configure()
    {
        ...
        if (strtolower($this->options['onetimeUrl']) === 'auto') {
            $this->options['onetimeUrl'] = fs_elFinder::getStaticVar('commonTempPath')? true : false;
        }
        ...
        if ($this->options['maxArcFilesSize']) {
            $this->options['maxArcFilesSize'] = fs_elFinder::getIniBytes('', $this->options['maxArcFilesSize']);
        }
     ...
     * Calls this function before umount this volume on fs_elFinder::exec()
     *
     * @return void
     */
    ...
    public function chmod($hash, $mode)
    {
        if ($this->commandDisabled('chmod')) {
            return $this->setError(fs_elFinder::ERROR_PERM_DENIED);
        }

        if (!($file = $this->file($hash))) {
            return $this->setError(fs_elFinder::ERROR_FILE_NOT_FOUND);
        }

        if (!$this->options['allowChmodReadOnly']) {
            if (!$this->attr($this->decode($hash), 'write', null, ($file['mime'] === 'directory'))) {
                return $this->setError(fs_elFinder::ERROR_PERM_DENIED, $file['name']);
            }
        }
    ...
    public function mount(array $opts)
    {
        ...
        $this->id = $this->driverId . (!empty($this->options['id']) ? $this->options['id'] : fs_elFinder::$volumesCnt++) . '_';
        ...
        if (!empty($this->options['uploadMaxSize'])) {
            $this->uploadMaxSize = fs_elFinder::getIniBytes('', $this->options['uploadMaxSize']);
        }
        ...
        $memLimit = fs_elFinder::getIniBytes('memory_limit') / 2;
        if ($memLimit > 0) {
            $this->getMaxSize = empty($this->options['getMaxSize']) ? $memLimit : min($memLimit, fs_elFinder::getIniBytes('', $this->options['getMaxSize']));
        } else {
            $this->getMaxSize = -1;
        }
        ...
        // set tmpLinkPath
        if (fs_elFinder::$tmpLinkPath && !$this->options['tmpLinkPath']) {
            if (is_writeable(fs_elFinder::$tmpLinkPath)) {
                $this->options['tmpLinkPath'] = fs_elFinder::$tmpLinkPath;
            } else {
                fs_elFinder::$tmpLinkPath = '';
            }
        }
        if ($this->options['tmpLinkPath'] && is_writable($this->options['tmpLinkPath'])) {
            ...
            if (!fs_elFinder::$tmpLinkPath) {
                fs_elFinder::$tmpLinkPath = $this->tmpLinkPath;
                fs_elFinder::$tmpLinkUrl = '';
            }
        }

        // set tmpLinkUrl
        if (fs_elFinder::$tmpLinkUrl && !$this->options['tmpLinkUrl']) {
            $this->options['tmpLinkUrl'] = fs_elFinder::$tmpLinkUrl;
        }
        ...
        if ($this->tmpLinkPath && !$this->tmpLinkUrl) {
            ...
            $this->tmpLinkUrl = ($https ? 'https://' : 'http://')
                ...
                . $uri;
            if (!fs_elFinder::$tmpLinkUrl) {
                fs_elFinder::$tmpLinkUrl = $this->tmpLinkUrl;
            }
        }
    ...
    public function file($hash)
    {
        ...
        return ($file) ? $file : $this->setError(fs_elFinder::ERROR_FILE_NOT_FOUND);
    }
    ...
    public function dir($hash, $resolveLink = false)
    {
        if (($dir = $this->file($hash)) == false) {
            return $this->setError(fs_elFinder::ERROR_DIR_NOT_FOUND);
        }
        ...
        return $dir && $dir['mime'] == 'directory' && empty($dir['hidden'])
            ? $dir
            : $this->setError(fs_elFinder::ERROR_NOT_DIR);
    }
    ...
    public function scandir($hash)
    {
        ...
        if ($res = $dir['read']
            ...
            : $this->setError(fs_elFinder::ERROR_PERM_DENIED)) {
    ...
    public function mkdir($dsthash, $name)
    {
        ...
        if (!$this->nameAccepted($name, true)) {
            return $this->setError(fs_elFinder::ERROR_INVALID_DIRNAME);
        }

        if (($dir = $this->dir($dsthash)) == false) {
            return $this->setError(fs_elFinder::ERROR_TRGDIR_NOT_FOUND, '#' . $dsthash);
        }
        ...
        if (!empty($stat)) {
            return $this->setError(fs_elFinder::ERROR_EXISTS, $name);
        }
        $this->clearcache();
    ...
    public function mkfile($dst, $name)
    {
        ...
        if (!$this->nameAccepted($name, false)) {
            return $this->setError(fs_elFinder::ERROR_INVALID_NAME);
        }

        $mimeByName = $this->mimetype($name, true);
        if ($mimeByName && !$this->allowPutMime($mimeByName)) {
            return $this->setError(fs_elFinder::ERROR_UPLOAD_FILE_MIME, $name);
        }

        if (($dir = $this->dir($dst)) == false) {
            return $this->setError(fs_elFinder::ERROR_TRGDIR_NOT_FOUND, '#' . $dst);
        }
    ...
    public function rename($hash, $name)
    {
        ...
        if (!empty($this->options['netkey']) && !empty($file['isroot'])) {
            ...
            if (fs_elFinder::$instance->updateNetVolumeOption($this->options['netkey'], 'alias', $name)) {
                $this->clearcache();
                $this->rootName = $this->options['alias'] = $name;
                return $this->stat($this->root);
            } else {
                return $this->setError(fs_elFinder::ERROR_TRGDIR_NOT_FOUND, $name);
            }
        }

        if (!empty($file['locked'])) {
            return $this->setError(fs_elFinder::ERROR_LOCKED, $file['name']);
        }

        $isDir = ($file['mime'] === 'directory');

        if (!$this->nameAccepted($name, $isDir)) {
            return $this->setError($isDir ? fs_elFinder::ERROR_INVALID_DIRNAME : fs_elFinder::ERROR_INVALID_NAME);
        }
    ...
    public function duplicate($hash, $suffix = 'copy')
    {
        if ($this->commandDisabled('duplicate')) {
            return $this->setError(fs_elFinder::ERROR_COPY, '#' . $hash, fs_elFinder::ERROR_PERM_DENIED);
        }

        if (($file = $this->file($hash)) == false) {
            return $this->setError(fs_elFinder::ERROR_COPY, fs_elFinder::ERROR_FILE_NOT_FOUND);
        }
    ...
    public function upload($fp, $dst, $name, $tmpname, $hashes = array())
    {
        ...
        if (!$this->allowPutMime($mime) || ($mimeByName && !$this->allowPutMime($mimeByName))) {
            return $this->setError(fs_elFinder::ERROR_UPLOAD_FILE_MIME, '(' . $mime . ')');
        }

        $tmpsize = (int)sprintf('%u', filesize($tmpname));
        if ($this->uploadMaxSize > 0 && $tmpsize > $this->uploadMaxSize) {
            return $this->setError(fs_elFinder::ERROR_UPLOAD_FILE_SIZE);
        }
        ...
        if ($file && $file['name'] === $name) { // file exists and check filename for item ID based filesystem
            if ($this->uploadOverwrite) {
                if (!$file['write']) {
                    ...
                    return $this->setError(fs_elFinder::ERROR_NOT_REPLACE, $name);
                }
                $this->remove($test);
            } else {
    ...
    {
        $err = $rmSrc ? fs_elFinder::ERROR_MOVE : fs_elFinder::ERROR_COPY;

        if ($this->commandDisabled('paste')) {
            return $this->setError($err, '#' . $src, fs_elFinder::ERROR_PERM_DENIED);
        }

        if (($file = $volume->file($src, $rmSrc)) == false) {
            return $this->setError($err, '#' . $src, fs_elFinder::ERROR_FILE_NOT_FOUND);
        }
        ...
        if (($dir = $this->dir($dst)) == false) {
            return $this->setError($err, $errpath, fs_elFinder::ERROR_TRGDIR_NOT_FOUND, '#' . $dst);
        }

        if (!$dir['write'] || !$file['read']) {
            return $this->setError($err, $errpath, fs_elFinder::ERROR_PERM_DENIED);
        }
        ...
        if (($test = $volume->closest($src, $rmSrc ? 'locked' : 'read', $rmSrc))) {
            return $rmSrc
                ? $this->setError($err, $errpath, fs_elFinder::ERROR_LOCKED, $volume->path($test))
                : $this->setError($err, $errpath, empty($file['thash']) ? fs_elFinder::ERROR_PERM_DENIED : fs_elFinder::ERROR_MKOUTLINK);
        }
        ...
        if ($stat && $stat['name'] === $name) { // file exists and check filename for item ID based filesystem
            if ($this->options['copyOverwrite']) {
                ...
                if (!$this->isSameType($file['mime'], $stat['mime'])) {
                    return $this->setError(fs_elFinder::ERROR_NOT_REPLACE, $this->path($stat['hash']));
                }
                ...
                if ($this->options['copyJoin']) {
                    if (!empty($stat['locked'])) {
                        return $this->setError(fs_elFinder::ERROR_LOCKED, $this->path($stat['hash']));
                    }
                } else {
                ...
                if ($volume === $this && ((isset($file['target']) && $test == $file['target']) || $test == $this->decode($src))) {
                    return $this->setError(fs_elFinder::ERROR_REPLACE, $errpath);
                }
                // remove existed file
                if (!$this->options['copyJoin'] || $stat['mime'] !== 'directory') {
                    if (!$this->remove($test)) {
                        return $this->setError(fs_elFinder::ERROR_REPLACE, $this->path($stat['hash']));
                    }
        ...
        if ($volume === $this) { //  changing == operand to === fixes issue #1285 - Paul Canning 24/03/2016
            ...
            if ($this->inpathCE($destination, $source)) {
                return $this->setError(fs_elFinder::ERROR_COPY_ITSELF, $errpath);
            }
            $rmDir = false;
            if ($rmSrc) {
                if ($dstDirExists) {
            ...
            if (!$this->options['copyTo'] || !$volume->copyFromAllowed()) {
                return $this->setError(fs_elFinder::ERROR_COPY, $errpath, fs_elFinder::ERROR_PERM_DENIED);
            }
            ...
            if ($rmSrc && !$this->error()) {
                if (!$volume->rm($src)) {
                    if ($volume->file($src)) {
                        $this->addError(fs_elFinder::ERROR_RM_SRC);
                    } else {
                        $this->removed[] = $file;
                    }
                }
    ...
    public function getContents($hash)
    {
        ...
        if ($file['mime'] == 'directory') {
            return $this->setError(fs_elFinder::ERROR_NOT_FILE);
        }
    ...
    public function putContents($hash, $content)
    {
        ...
        if (!$this->allowPutMime($mimeByName) || ($mime && !$this->allowPutMime($mime))) {
            return $this->setError(fs_elFinder::ERROR_UPLOAD_FILE_MIME);
        }
    ...
    public function extract($hash, $makedir = null)
    {
        ...
        if (!$archiver) {
            return $this->setError(fs_elFinder::ERROR_NOT_ARCHIVE);
        }
    ...
    public function archive($hashes, $mime, $name = '')
    {
        ...
        if (!$archiver) {
            return $this->setError(fs_elFinder::ERROR_ARCHIVE_TYPE);
        }
        ...
        foreach ($hashes as $hash) {
            if (($file = $this->file($hash)) == false) {
                return $this->setError(fs_elFinder::ERROR_FILE_NOT_FOUND, '#' . $hash);
            }
            if (!$file['read']) {
                return $this->setError(fs_elFinder::ERROR_PERM_DENIED);
            }
    ...
    public function resize($hash, $width, $height, $x, $y, $mode = 'resize', $bg = '', $degree = 0, $jpgQuality = null)
    {
        ...
        if ($this->imgLib !== 'imagick' && $this->imgLib !== 'convert') {
            if (fs_elFinder::isAnimationGif($work_path)) {
                return $this->setError(fs_elFinder::ERROR_UNSUPPORT_TYPE);
            }
        }

        if (fs_elFinder::isAnimationPng($work_path)) {
            return $this->setError(fs_elFinder::ERROR_UNSUPPORT_TYPE);
        }
    ...
    public function getContentUrl($hash, $options = array())
    {
        ...
        if (!empty($options['onetime']) && $this->options['onetimeUrl']) {
            if (is_callable($this->options['onetimeUrl'])) {
                ...
                $ret = false;
                if ($tmpdir = fs_elFinder::getStaticVar('commonTempPath')) {
                            ...
                            if (rename($_dat, $dat)) {
                                ...
                                if ($tmp) {
                                    ...
                                    if (file_put_contents($dat, json_encode($info))) {
                                        $conUrl = fs_elFinder::getConnectorUrl();
    ...
    public function getTempLinkInfo($name = null)
    {
        if ($this->tmpLinkPath) {
            ...
            register_shutdown_function(array('elFinder', 'GlobGC'), $this->tmpLinkPath . DIRECTORY_SEPARATOR . 'temp_*', fs_elFinder::$tmpLinkLifeTime);
            return array(
                    ...
                    if ($srcfp && ($tempLink = $this->getTempLinkInfo())) {
                        ...
                        if ($dest && stream_copy_to_stream($srcfp, $dest)) {
                            ...
                            if ($this->imageUtil('resize', $tempLink['path'], compact('width', 'height', 'jpgQuality', 'preserveExif', 'unenlarge', 'checkAnimated', 'destformat'))) {
                                ...
                                touch($tempLink['path'], time() - fs_elFinder::$tmpLinkLifeTime + 60);
                            } else {
    ...
    public function getTempPath()
    {
        ...
        if (isset($this->tmpPath) && $this->tmpPath && is_writable($this->tmpPath)) {
            ...
            $tempPath = $this->tmp;
        } else if (fs_elFinder::getStaticVar('commonTempPath') && is_writable(fs_elFinder::getStaticVar('commonTempPath'))) {
            $tempPath = fs_elFinder::getStaticVar('commonTempPath');
    ...
    public function imageUtil($mode, $src, $options = array())
    {
        ...
        if (!empty($options['checkAnimated'])) {
            if ($this->imgLib !== 'imagick' && $this->imgLib !== 'convert') {
                if (fs_elFinder::isAnimationGif($src)) {
                    return false;
                }
            }
            if (fs_elFinder::isAnimationPng($src)) {
                return false;
            }
        }
    ...
    public function uniqueName($dir, $name, $suffix = ' copy', $checkNum = true, $start = 1)
    {
        ...
        $splits = fs_elFinder::splitFileExtention($name);
        if ($splits[1]) {
            $ext = '.' . $splits[1];
            $name = $splits[0];
        }
    ...
    protected function convEnc($var, $from, $to, $locale, $restoreLocale, $unknown = '_')
    {
        if (strtoupper($from) !== strtoupper($to)) {
            ...
            if ($restoreLocale) {
                setlocale(LC_ALL, fs_elFinder::$locale);
            }
        }
        return $var;
    }
        ...
        foreach ($hashes as $hash) {
            ...
            if ($file['mime'] === 'directory') {
                ...
                if ($this->options['maxArcFilesSize'] > 0 && $this->options['maxArcFilesSize'] < $totalSize) {
                    $res = $this->setError(fs_elFinder::ERROR_ARC_MAXSIZE);
                }
            }
        }
    ...
    static protected function loadMimeTypes($mimeTypesFile = '')
    {
        if (!elFinderVolumeDriver::$mimetypesLoaded) {
            ...
            if (!empty($mimeTypesFile) && file_exists($mimeTypesFile)) {
                $file = $mimeTypesFile;
            } elseif (fs_elFinder::$defaultMimefile && file_exists(fs_elFinder::$defaultMimefile)) {
                $file = fs_elFinder::$defaultMimefile;
    ...
    protected function doSearch($path, $q, $mimes)
    {
        ...
        if ($timeout && $timeout < time()) {
            $this->setError(fs_elFinder::ERROR_SEARCH_TIMEOUT, $this->path($this->encode($path)));
            return $result;
        }

        foreach ($this->scandirCE($path) as $p) {
            fs_elFinder::extendTimeLimit($this->options['searchTimeout'] + 30);

            if ($timeout && ($this->error || $timeout < time())) {
                !$this->error && $this->setError(fs_elFinder::ERROR_SEARCH_TIMEOUT, $this->path($this->encode($path)));
                break;
            }
    ...
    protected function copy($src, $dst, $name)
    {
        ...
        if (!empty($srcStat['thash'])) {
            ...
            if (!$this->inpathCE($target, $this->root)) {
                return $this->setError(fs_elFinder::ERROR_COPY, $this->path($srcStat['hash']), fs_elFinder::ERROR_MKOUTLINK);
            }
            $stat = $this->stat($target);
            $this->clearcache();
            return $stat && $this->symlinkCE($target, $dst, $name)
                ? $this->joinPathCE($dst, $name)
                : $this->setError(fs_elFinder::ERROR_COPY, $this->path($srcStat['hash']));
        }
        ...
        if ($srcStat['mime'] === 'directory') {
            ...
            if (($testStat && $testStat['mime'] !== 'directory') || (!$testStat && !$testStat = $this->mkdir($this->encode($dst), $name))) {
                return $this->setError(fs_elFinder::ERROR_COPY, $this->path($srcStat['hash']));
            }
            ...
            foreach ($this->getScandir($src) as $stat) {
                if (empty($stat['hidden'])) {
                    ...
                    if (!$this->copy($_src, $dst, $name)) {
                        ...
                        return $this->setError($this->error, fs_elFinder::ERROR_COPY, $this->_path($src));
                    }
                }
            }
    ...
    protected function move($src, $dst, $name)
    {
        ...
        return $this->setError(fs_elFinder::ERROR_MOVE, $this->path($stat['hash']));
    }
    ...
    protected function copyFrom($volume, $src, $destination, $name)
    {
        ...
        if (($source = $volume->file($src)) == false) {
            return $this->addError(fs_elFinder::ERROR_COPY, '#' . $src, $volume->error());
        }
        ...
        try {
            ...
            fs_elFinder::$instance->trigger('paste.copyfrom', array(&$thash, &$name, '', fs_elFinder::$instance, $this), $errors);
        } catch (elFinderTriggerException $e) {
            return $this->addError(fs_elFinder::ERROR_COPY, $name, $errors);
        }

        if (!$this->nameAccepted($name, $srcIsDir)) {
            return $this->addError(fs_elFinder::ERROR_COPY, $name, $srcIsDir ? fs_elFinder::ERROR_INVALID_DIRNAME : fs_elFinder::ERROR_INVALID_NAME);
        }

        if (!$this->allowCreate($destination, $name, $srcIsDir)) {
            return $this->addError(fs_elFinder::ERROR_COPY, $name, fs_elFinder::ERROR_PERM_DENIED);
        }

        if (!$source['read']) {
            return $this->addError(fs_elFinder::ERROR_COPY, $errpath, fs_elFinder::ERROR_PERM_DENIED);
        }
        ...
        if ($srcIsDir) {
            ...
            if (($test && $test['mime'] != 'directory') || (!$test && !$test = $this->mkdir($this->encode($destination), $name))) {
                return $this->addError(fs_elFinder::ERROR_COPY, $errpath);
            }
            ...
            if (!$this->allowPutMime($source['mime']) || ($mimeByName && !$this->allowPutMime($mimeByName))) {
                return $this->addError(fs_elFinder::ERROR_UPLOAD_FILE_MIME, $errpath);
            }
    ...
    protected function remove($path, $force = false)
    {
        ...
        if (empty($stat)) {
            return $this->setError(fs_elFinder::ERROR_RM, $path, fs_elFinder::ERROR_FILE_NOT_FOUND);
        }
        ...
        if ($stat['mime'] == 'directory' && empty($stat['thash'])) {
            ...
            $this->convEncOut();
            if (!$ret) {
                return $this->setError(fs_elFinder::ERROR_RM, $this->path($stat['hash']));
            }
        } else {
    ...
    protected function imgResize($path, $width, $height, $keepProportions = false, $resizeByBiggerSide = true, $destformat = null, $jpgQuality = null, $options = array())
    {
        ...
        fs_elFinder::extendTimeLimit(300);
        switch ($this->imgLib) {
            case 'imagick':
            ...
            case 'gd':
                fs_elFinder::expandMemoryForGD(array($s, array($size_w, $size_h)));
    ...
    protected function imgCrop($path, $width, $height, $x, $y, $destformat = null, $jpgQuality = null)
    {
        ...
        switch ($this->imgLib) {
            ...
            case 'gd':
                fs_elFinder::expandMemoryForGD(array($s, array($width, $height)));
                $img = $this->gdImageCreate($path, $s['mime']);
    ...
    protected function imgRotate($path, $degree, $bgcolor = '#ffffff', $destformat = null, $jpgQuality = null)
    {
        ...
        switch ($this->imgLib) {
            ...
            case 'gd':
                fs_elFinder::expandMemoryForGD(array($s, $s));
                $img = $this->gdImageCreate($path, $s['mime']);
    ...
    {
        return fs_elFinder::procExec($command, $output, $return_var, $error_output);
    }
        ...
        if ($this->tmbPathWritable) {
            if ($stat['mime'] === 'directory') {
                foreach ($this->scandirCE($this->decode($stat['hash'])) as $p) {
                    fs_elFinder::extendTimeLimit(30);
    ...
    protected function getArchivers($use_cache = true)
    {
        ...
        if ($use_cache) {
            if (isset($this->options['archivers']) && is_array($this->options['archivers']) && $this->options['archivers']) {
                ...
                $cache = fs_elFinder::$archivers;
            }
            if ($cache) {
                return $cache;
            } else {
                if ($cache = $this->session->get($sessionKey, array())) {
                    return fs_elFinder::$archivers = $cache;
                }
            }
        }
        ...
        return fs_elFinder::$archivers = $arcs;
    }
                        ...
                        if ($o) {
                            ...
                            if (!empty($this->options['maxArcFilesSize'])) {
                                if ($comp($size, (string)$this->options['maxArcFilesSize']) > 0) {
                                    throw new Exception(fs_elFinder::ERROR_ARC_MAXSIZE);
                                }
                            }
                        }
                        unset($o, $c);
    ...
    protected function checkExtractItems($path, $checks = null)
    {
        ...
        foreach ($files as $name) {
            ...
            $utf8Name = fs_elFinder::$instance->utf8Encode($name);
            if ($name !== $utf8Name) {
                $fsSame = false;
                if ($this->encoding) {
                    // test as fs encoding

...
class fs_elFinder
{
    ...
    public function __construct($opts)
    {
        ...
        set_error_handler('fs_elFinder::phpErrorHandler', $errLevel);
        ...
        fs_elFinder::$commonTempPath = (isset($opts['commonTempPath']) ? realpath($opts['commonTempPath']) : dirname(__FILE__) . '/.tmp');
        if (!is_writable(fs_elFinder::$commonTempPath)) {
            fs_elFinder::$commonTempPath = sys_get_temp_dir();
            if (!is_writable(fs_elFinder::$commonTempPath)) {
                fs_elFinder::$commonTempPath = '';
            }
        }
        if (isset($opts['connectionFlagsPath']) && is_writable($opts['connectionFlagsPath'] = realpath($opts['connectionFlagsPath']))) {
            fs_elFinder::$connectionFlagsPath = $opts['connectionFlagsPath'];
        } else {
            fs_elFinder::$connectionFlagsPath = fs_elFinder::$commonTempPath;
        }

        if (!empty($opts['tmpLinkPath'])) {
            fs_elFinder::$tmpLinkPath = realpath($opts['tmpLinkPath']);
        }
        if (!empty($opts['tmpLinkUrl'])) {
            fs_elFinder::$tmpLinkUrl = $opts['tmpLinkUrl'];
        }
        if (!empty($opts['tmpLinkLifeTime'])) {
            fs_elFinder::$tmpLinkLifeTime = $opts['tmpLinkLifeTime'];
        }
        if (!empty($opts['textMimes']) && is_array($opts['textMimes'])) {
            fs_elfinder::$textMimes = $opts['textMimes'];
        }
        ...
        // setlocale and global locale regists to fs_elFinder::locale
        ...
        // set defaultMimefile
        fs_elFinder::$defaultMimefile = isset($opts['defaultMimefile']) ? $opts['defaultMimefile'] : '';

        // set memoryLimitGD
        fs_elFinder::$memoryLimitGD = isset($opts['memoryLimitGD']) ? $opts['memoryLimitGD'] : 0;
        ...
        // set archivers
        fs_elFinder::$archivers = isset($opts['archivers']) && is_array($opts['archivers']) ? $opts['archivers'] : array();
        ...
        if (empty(fs_elFinder::$netDrivers['ziparchive'])) {
            fs_elFinder::$netDrivers['ziparchive'] = 'FlysystemZipArchiveNetmount';
        }
    ...
    {
        // set error handler of WARNING, NOTICE
        set_error_handler('fs_elFinder::phpErrorHandler', E_WARNING | E_NOTICE | E_USER_WARNING | E_USER_NOTICE);
        ...
        if ($this->sessionCloseEarlier && $args['sessionCloseEarlier']) {
            $this->session->close();
            // deprecated property
            fs_elFinder::$sessionClosed = true;
        }
        ...
        if (!empty($result['debug'])) {
            $result['debug']['backendErrors'] = fs_elFinder::$phpErrors;
        }
        fs_elFinder::$phpErrors = array();
        restore_error_handler();
    ...
    protected function open($args)
    {
        ...
        if ($args['compare']) {
            ...
            do {
                fs_elFinder::extendTimeLimit(30 + $sleep);
                $_mtime = 0;
                foreach ($ls as $_f) {
                    if (isset($_f['ts'])) {
        ...
        if($cwd['name'] == 'Shared' && $cwd['mime'] == 'directory'){
            $CI = & get_instance();
            $files = $CI->file_sharing_model->get_file_share($cwd['hash']);
        }elseif ($cwd['name'] == 'Client Files' && $cwd['mime'] == 'directory') {
            if(is_client_logged_in()){
                foreach ($files as $key => $value) {
                    if($value['name'] != str_pad(get_client_user_id(), 5, '0', STR_PAD_LEFT)){
                        unset($files[$key]);
                    }
                }
            }
        ...
        if (!empty($args['init'])) {
            ...
            if (fs_elfinder::$textMimes) {
                $result['textMimes'] = fs_elfinder::$textMimes;
            }
        }
    ...
    protected function tree($args)
    {
        ...
        if(is_client_logged_in()){
            ...
            foreach ($tree as $key => $value) {
                if($value['name'] == 'Client Files' && $value['mime'] == 'directory'){
                    $h = substr($value['hash'], 3);
            ...
            if($h != ''){
                foreach ($tree as $key => $value) {
                    if($value['phash'] == $ha && $value['name'] != str_pad(get_client_user_id(), 5, '0', STR_PAD_LEFT)){
                        unset($tree[$key]);
                    }
                }
            }
    ...
    protected function file($args)
    {
        ...
        if ($onetime) {
            $volume = null;
            $tmpdir = fs_elFinder::$commonTempPath;
    ...
    protected function rename($args)
    {
        ...
        if ($targets) {
            ...
            if ($query) {
                // batch rename
                $splits = fs_elFinder::splitFileExtention($query);
            ...
            foreach ($rms as $rm) {
                if ($type === 'normal') {
                    ...
                    $rname = $name;
                    if ($type === 'extention') {
                        $splits = fs_elFinder::splitFileExtention($rm['name']);
                        $rname = $splits[0] . '.' . $name;
                    } else if ($type === 'prefix') {
                        $rname = $name . $rm['name'];
                    } else if ($type === 'suffix') {
    ...
    {
        if (!fs_elFinder::$connectionFlagsPath || $_SERVER['REQUEST_METHOD'] === 'HEAD') {
            return;
        }
        $flagFile = fs_elFinder::$connectionFlagsPath . DIRECTORY_SEPARATOR . 'elfreq%s';
    ...
    private function getTempDir($volumeTempPath = null)
    {
        ...
        if (fs_elFinder::$commonTempPath) {
            $testDirs[] = fs_elFinder::$commonTempPath;
        }
        $tempDir = '';
...
';
    ...
    public static function phpErrorHandler($errno, $errstr, $errfile, $errline)
    {
        ...
        switch ($errno) {
            case E_WARNING:
            case E_USER_WARNING:
                fs_elFinder::$phpErrors[] = "WARNING: $errstr in $errfile line $errline.";
                $proc = true;
                break;

            case E_NOTICE:
            case E_USER_NOTICE:
                fs_elFinder::$phpErrors[] = "NOTICE: $errstr in $errfile line $errline.";
                $proc = true;
                break;

            case E_STRICT:
                fs_elFinder::$phpErrors[] = "STRICT: $errstr in $errfile line $errline.";
                $proc = true;
                break;

            case E_RECOVERABLE_ERROR:
                fs_elFinder::$phpErrors[] = "RECOVERABLE_ERROR: $errstr in $errfile line $errline.";
                $proc = true;
                break;
        }
        ...
        if (defined('E_DEPRECATED')) {
            switch ($errno) {
                ...
                case E_USER_DEPRECATED:
                    fs_elFinder::$phpErrors[] = "DEPRECATED: $errstr in $errfile line $errline.";
                    $proc = true;
                    break;
            }
        }
    ...
    protected function itemLocked($hash)
    {
        ...
        $lock = fs_elFinder::$commonTempPath . DIRECTORY_SEPARATOR . $hash . '.lock';
        if (file_exists($lock)) {
    ...
    {
        $id = (int)$resource;
        if (isset(fs_elFinder::$seekableUrlFps[$id])) {
            return fs_elFinder::$seekableUrlFps[$id];
        }
        return null;
    }
    ...
    {
        return isset(fs_elFinder::$$key) ? fs_elFinder::$$key : null;
    }
        ...
        if ($dlurl) {
            ...
            while (($res = trim(fgets($stream))) !== '') {
                ...
                if (preg_match('/^(Accept-Ranges|Content-Range): bytes/i', $res)) {
                    fs_elFinder::$seekableUrlFps[(int)$stream] = true;
                }
            }
            if ($data['url']) {
                ++$data['cnt'];
    ...
    {
        if (fs_elFinder::$memoryLimitGD != 0 && $imgInfos && is_array($imgInfos)) {
            if (!is_array($imgInfos[0])) {
                $imgInfos = array($imgInfos);
            }
            $limit = self::getIniBytes('', fs_elFinder::$memoryLimitGD);
 ...
 */
class fs_elFinderAbortException extends Exception
{
}

class fs_elFinderTriggerException extends Exception
{
}
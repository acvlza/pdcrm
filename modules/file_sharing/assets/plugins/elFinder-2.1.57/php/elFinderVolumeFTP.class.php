...
class elFinderVolumeFTP extends elFinderVolumeDriver
{
    ...
    protected function _extract($path, $arc)
    {
        ...
        if ($checkRes['symlinks']) {
            $this->rmdirRecursive($dir);
            return $this->setError(array_merge($this->error, array(fs_elFinder::ERROR_ARC_SYMLINKS)));
        }
        $this->archiveSize = $checkRes['totalSize'];
        if ($checkRes['rmNames']) {
            foreach ($checkRes['rmNames'] as $name) {
                $this->addError(fs_elFinder::ERROR_SAVE, $name);
            }
        }
        ...
        if ($this->options['maxArcFilesSize'] > 0 && $this->options['maxArcFilesSize'] < $this->archiveSize) {
            $this->rmdirRecursive($dir);
            return $this->setError(fs_elFinder::ERROR_ARC_MAXSIZE);
        }
        ...
        if (($extractTo === 'auto' || !$extractTo) && count($filesToProcess) === 1 && is_file($src)) {
            ...
            $src = $dir;
            $splits = fs_elFinder::splitFileExtention(basename($path));
            $name = $splits[0];
    ...
    private function tempDir()
    {
        ...
        if (!$tempPath) {
            $this->setError(fs_elFinder::ERROR_CREATING_TEMP_DIR, $this->tmp);
            return false;
        }
    ...
    private function ftp_download_files($remote_directory, array $files, $dest_local_directory)
    {
        ...
        if (!isset($contents)) {
            $this->setError(fs_elFinder::ERROR_FTP_DOWNLOAD_FILE, $remote_directory);
            return false;
        }
    ...
    private function deleteDir($dirPath)
    {
        ...
        if (!$success) {
            $this->setError(fs_elFinder::ERROR_RM, $dirPath);
            return false;
        }
        return $success;
    }

...
class elFinderVolumeGoogleDrive extends elFinderVolumeDriver
{
    ...
    /**
     * Prepare
     * Call from fs_elFinder::netmout() before volume->mount().
     *
     * @return array
     * @author Naoki Sawada
    ...
    public function netmountPrepare($options)
    {
        ...
        try {
            ...
            if ($aToken) {
                try {
                    ...
                    if ($options['user'] !== 'init') {
                        ...
                        return ['exit' => true, 'error' => fs_elFinder::ERROR_REAUTH_REQUIRE];
                    }
                }
            }
            ...
            if ($code || (isset($options['user']) && $options['user'] === 'init')) {
                if (empty($options['url'])) {
                    $options['url'] = fs_elFinder::getConnectorUrl();
                }
                ...
                if (!$aToken && empty($code)) {
                    ...
                    if (empty($options['pass']) && $options['host'] !== '1') {
                        ...
                        $out = [
                            'node' => $options['id'],
                            'json' => '{"protocol": "googledrive", "mode": "makebtn", "body" : "' . str_replace($html, '"', '\\"') . '", "error" : "' . fs_elFinder::ERROR_ACCESS_DENIED . '"}',
                            'bind' => 'netmount',
                        ];
            ...
            if (empty($options['pass'])) {
                return ['exit' => true, 'body' => '{msg:' . fs_elFinder::ERROR_ACCESS_DENIED . '}' . ' ' . $e->getMessage()];
            } else {
                return ['exit' => true, 'error' => [fs_elFinder::ERROR_ACCESS_DENIED, $e->getMessage()]];
            }
        }
        ...
        try {
            ...
            if (isset($err['error']) && $err['error']['code'] == 404) {
                return ['exit' => true, 'error' => [fs_elFinder::ERROR_TRGDIR_NOT_FOUND, $options['path']]];
            } else {
    ...
    protected function init()
    {
        ...
        try {
            if (!$serviceAccountConfig) {
                ...
                if ($this->options['refresh_token']) {
                    ...
                    $tmp = fs_elFinder::getStaticVar('commonTempPath');
                    if (!$tmp) {
                ...
                if (!$serviceAccountConfig) {
                    ...
                    if ($client->isAccessTokenExpired()) {
                        ...
                        if (!empty($this->options['netkey'])) {
                            fs_elFinder::$instance->updateNetVolumeOption($this->options['netkey'], 'access_token', $access_token);
                        }
                        $this->options['access_token'] = $access_token;
                    }
        ...
        if (empty($this->options['alias'])) {
            if ($this->needOnline) {
                ...
                if (!empty($this->options['netkey'])) {
                    fs_elFinder::$instance->updateNetVolumeOption($this->options['netkey'], 'alias', $this->options['alias']);
                }
            } else {
        ...
        if (!$this->tmp && ($tmp = fs_elFinder::getStaticVar('commonTempPath'))) {
            $this->tmp = $tmp;
        }
    ...
    protected function doSearch($path, $q, $mimes)
    {
        ...
        foreach ($res as $raw) {
            if ($timeout && $timeout < time()) {
                $this->setError(fs_elFinder::ERROR_SEARCH_TIMEOUT, $this->_path($path));
                break;
            }
            if ($stat = $this->_gd_parseRaw($raw)) {
                if ($parents = $raw->getParents()) {
    ...
    protected function copy($src, $dst, $name)
    {
        ...
        if ($res['mimeType'] == self::DIRMIME) {
            ...
            if ($newDir) {
                ...
                $ret = $this->setError(fs_elFinder::ERROR_COPY, $this->_path($src));
            }
        } else {
    ...
    protected function remove($path, $force = false, $recursive = false)
    {
        ...
        if (empty($stat)) {
            return $this->setError(fs_elFinder::ERROR_RM, $this->_path($path), fs_elFinder::ERROR_FILE_NOT_FOUND);
        }

        if (!$force && !empty($stat['locked'])) {
            return $this->setError(fs_elFinder::ERROR_LOCKED, $this->_path($path));
        }

        if ($stat['mime'] == 'directory') {
            if (!$recursive && !$this->_rmdir($path)) {
                return $this->setError(fs_elFinder::ERROR_RM, $this->_path($path));
            }
        } else {
                ...
                if (!empty($args['substitute'])) {
                    ...
                    if ($srcSize[0] && $srcSize[1]) {
                        if (min(($tmbSize / $srcSize[0]), ($tmbSize / $srcSize[1])) < 1) {
                            ...
                            } elseif ($subImgLink = $this->getSubstituteImgLink(fs_elFinder::$currentArgs['target'], $srcSize)) {
                                $ret['url'] = $subImgLink;
                            }
                        }
                    }
            ...
            if ($file = $this->_gd_getFile($path)) {
                if ($dlurl = $this->_gd_getDownloadUrl($file)) {
                    ...
                    if ($access_token) {
                        ...
                        return fs_elFinder::getStreamByUrl($data);
                    }
                }
            }
        }
    ...
    protected function _save($fp, $path, $name, $stat)
    {
        ...
        try {
            ...
            if ($size > 0) {
                $memory = fs_elFinder::getIniBytes('memory_limit');
                if ($memory > 0) {

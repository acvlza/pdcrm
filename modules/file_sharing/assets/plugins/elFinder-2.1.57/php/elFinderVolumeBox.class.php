...
class elFinderVolumeBox extends elFinderVolumeDriver
{
    ...
    protected function _bd_refreshToken()
    {
        if (!property_exists($this->token, 'expires') || $this->token->expires < time()) {
            ...
            if (empty($this->token->data->refresh_token)) {
                throw new \Exception(fs_elFinder::ERROR_REAUTH_REQUIRE);
            } else {
            ...
            if (empty($decoded->access_token)) {
                ...
                throw new \Exception($err? $err : fs_elFinder::ERROR_REAUTH_REQUIRE);
            }
            ...
            if (!empty($decoded->refresh_token)) {
                if (empty($this->options['netkey']) && $aTokenFile) {
                    ...
                    fs_elFinder::$instance->updateNetVolumeOption($this->options['netkey'], 'accessToken', $json);
    ...
    protected function _bd_curlExec($curl, $decodeOrParent = true, $headers = array(), $postData = array())
    {
        ...
        $result = fs_elFinder::curlExec($curl, array(), $headers, $postData);
    ...
    public function netmountPrepare($options)
    {
        ...
        try {
            ...
            if ($code) {
                ...
                $out = array(
                    'node' => $options['id'],
                    'json' => json_encode(array('error' => fs_elFinder::ERROR_ACCESS_DENIED)),
                );
            ...
            if ($options['user'] === 'init') {
                ...
                if ($result === false) {
                    $redirect = fs_elFinder::getConnectorUrl();
        ...
        if ($_aToken = $this->session->get('BoxTokens')) {
            ...
            $this->session->remove('BoxTokens');
            $this->setError(fs_elFinder::ERROR_NETMOUNT, $options['host'], implode(' ', $this->error()));
    ...
    protected function copy($src, $dst, $name)
    {
        if ($res = $this->_copy($src, $dst, $name)) {
            ...
            return $this->setError(fs_elFinder::ERROR_COPY, $this->_path($src));
        }
    }
    ...
    protected function _dimensions($path, $mime)
    {
        ...
        if ($work = $this->getWorkFile($path)) {
            if ($size = @getimagesize($work)) {
                ...
                $srcfp = fopen($work, 'rb');
                $target = isset(fs_elFinder::$currentArgs['target'])? fs_elFinder::$currentArgs['target'] : '';

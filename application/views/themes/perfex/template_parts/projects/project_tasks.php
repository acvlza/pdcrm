...
<div class="pull-right tasks-table<?php if (
    $project->settings->view_milestones == 1
) {
    echo " hide";
} ?>">
    <select class="selectpicker" name="tasksFilterByStatus[]"
        ...
        <option value="<?php echo $status["name"]; ?>"><?php echo $status[
    "name"
]; ?></option>
        <?php } ?>
    </select>
...
<div class="tasks-phases">
    ...
    $milestones[] = [
        ...
        "id" => 0,
        "total_logged_time" => $this->projects_model->calc_milestone_logged_time(
            $project->id,
            0
        ),
        "total_tasks" => total_project_tasks_by_milestone(0, $project->id),
        "total_finished_tasks" => total_project_finished_tasks_by_milestone(
            0,
            $project->id
        ),
        "color" => null,
    ...
    <div class="kan-ban-row">
        <?php foreach ($milestones as $milestone) {
            ...
            $tasks = $this->projects_model->get_tasks($project->id, [
                "milestone" => $milestone["id"],
            ]);
            $percent = 0;
            if (
                $milestone["total_finished_tasks"] >=
                floatval($milestone["total_tasks"])
            ) {
                $percent = 100;
            } else {
                if ($milestone["total_tasks"] !== 0) {
                    $percent = number_format(
                        ($milestone["total_finished_tasks"] * 100) /
        ...
        <div class="kan-ban-col<?php if (
            ...
            ?>" <?php echo $milestone_color; ?>>
                ...
                <?php } ?>
                <span class="bold tw-text-sm"><?php echo $milestone[
                    "name"
                ]; ?></span>
                <span class="tw-text-xs">
                    <?php echo $milestone["id"] != 0
                        ? " | " .
            ...
            <div class="panel-body">
                ...
                <div class="media _task_wrapper<?php if (
                    !empty($task["duedate"]) &&
                    ...
                    <div class="media-body">
                        <a href="<?php echo site_url(
                            "clients/project/" .
                                $project->id .
                                "?group=project_tasks&taskid=" .
                                $task["id"]
                        ); ?>"
                            class="task_milestone tw-mb-1 pull-left<?php if (
                                $task["status"] == Tasks_model::STATUS_COMPLETE
                            ) {
                        ...
                        <p class="text-xs tw-mb-0">
                            <?php echo format_task_status(
                                $task["status"],
                                true
                            ); ?>
                        </p>
                        <p class="tw-mb-0 tw-text-xs tw-text-neutral-500"><?php echo _l(
                            "tasks_dt_datestart"
                        ); ?>:
                        ...
                        <p class="tw-mb-0 tw-text-xs tw-text-neutral-500">
                            <?php echo _l("task_duedate"); ?>: <b><?php echo _d(
    $task["duedate"]
); ?></b>
                        </p>
                        <?php } ?>
...
<div class="tasks-table<?php if ($project->settings->view_milestones == 1) {
    ...
    <table class="table dt-table table-tasks" data-order-col="3" data-s-type='[{"column":3,"type":"task-status"}]'
        ...
        <tbody>
            ...
            <tr>
                ...
                <td data-order="<?php echo $task[
                    "startdate"
                ]; ?>"><?php echo _d($task["startdate"]); ?></td>
                <td data-order="<?php echo $task["duedate"]; ?>">
                    <?php
                        echo _d($task["duedate"]); // Extract the task status
                ...
                <td data-order="<?php echo $taskStatus; ?>" style="text-align:center; vertical-align:middle;">
                    <span class="<?php echo $labelStyles; ?>">
    ...
    echo $task["milestone_name"];
            ...
            <!--    <td data-order="<?php echo $task["billable"]; ?>">
                ...
                <td data-order="<?php echo $task["billed"]; ?>">
                    ...
                    if ($task["billed"] == 1) {
                        ...
                        $billed =
                            ...
                            ($task["billable"] == 1 ? "danger" : "default") .
                            ' pull-left">' .
                            _l("task_billed_no") .
                            "</span>";
                    }
                ...
                <td><?php echo get_custom_field_value(
                    $task["id"],
                    $field["id"],
                    "tasks"
                ); ?></td>
...
<?php } else {get_template_part("projects/project_task");}} ?>
<script>
$(function() {

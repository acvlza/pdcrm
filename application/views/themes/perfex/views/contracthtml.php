        ...
        <div class="container preview-sticky-container">
            <div class="sm:tw-flex sm:tw-justify-between -tw-mx-4">
                <div class="sm:tw-self-end tw-inline-flex">
                    ...
                    <span class="label label-success -tw-mt-1 tw-self-start tw-ml-4 content-view-status contract-html-is-signed">APPROVED</span>
                    <?php } ?>
                    
                
<?php 
$this->db->where(['status'=>1,'project_id'=>$contract->project_id]);
$result = $this->db->get(db_prefix().'invoices')->row();
if($result){?>              
<a target="_blank" href="<?php echo site_url('invoice/'.$result->id.'/'.$result->hash); ?>"><span class="label label-paynow -tw-mt-1 tw-self-start tw-ml-4 content-view-status contract-html-is-signed">PAY PROJECT</span></a>
<?php }?>                   
            ...
            <div class="panel-body tc-content contract-html-content">
                ...
                <table>
                    ...
                    <tr> 
                        ...
                        <td style="color: #9ba1c4!important;"><?php echo get_project_name_by_id($contract->project_id); ?></td>
                        <td width="30%"></td>
                        <td><b>Document ID:</b></td>
                        <td style="color: #9ba1c4!important;"><?php echo $contract->id; ?></td>
                    </tr>
                    <tr> 
                        <td><b>Client Name:</b></td>
                        <td style="color: #9ba1c4!important;"><?php echo get_customer_name_by_project_id($contract->project_id); ?></td>
                        <td width="30%"></td>
                        <td><b>Date Accepted:</b></td>
                        <td style="color: #9ba1c4!important;"><?php echo _l('date_signed'); ?></td>
                    </tr>
                    <tr> 
                        <td><b>Portier Digital Contact:</b></td>
                        <td style="color: #9ba1c4!important;"><?php echo get_staff_full_name($estimate->sale_agent); ?></td>
                        <td width="30%">
                        <td><b>Project Schedule:</b> </td>
                        <td style="color: #9ba1c4!important;"><?php echo date('F d, Y', strtotime(get_project_startdate($contract->project_id))); ?> - <?php echo date('F d, Y', strtotime(get_project_deadline($contract->project_id))); ?></td>
                    </tr>
                    <tr> 
                        <td></td>
                        <td></td>
                        ...
                        <td style="color: #9ba1c4!important;"><?php echo app_format_money($contract->contract_value, get_base_currency()); ?></td>
                    </tr>
...
<script>
function approve_milestone(milestone_id) {
$.post("<?php echo site_url('video_library/client_video_requests/approve_milestone'); ?>", {
id: milestone_id,
}, function(result){
    //alert(result);
if(result == 'success'){
...
function decline_milestone(milestone_id) {
    
$.post("<?php echo site_url('video_library/client_video_requests/decline_milestone'); ?>", {
id: milestone_id,
}, function(result){
    //alert(result);
if(result == 'success'){
alert('Milestone Declined');
location.reload(); 
//$('#ModalConfirm').modal('show');
}else{
alert('An error occured');

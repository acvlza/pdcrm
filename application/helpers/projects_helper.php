...
/**
 * Return a milestone name linked to the project ID
 * @param  mixed $project_id project id
 * @return string
 */
function get_project_milestone_name($project_id)
{
    $CI = &get_instance();
    $CI->db->where('project_id', $project_id);
    $CI->db->order_by('milestone_order', 'ASC');
    $milestones = $CI->db->get(db_prefix() . 'milestones')->result_array();

    $result = '';

    foreach ($milestones as $milestone) {
        // Generate milestone name
        $milestoneName = generate_milestone_name();
        
        // Append the milestone name to the result variable
...
/**
 * Return milestone names, associated tasks, and schedule linked to the project ID
 * @param mixed $project_id project ID
 * @return string
 */
function get_project_milestone_schedule_inscope($project_id)
{
    $CI = &get_instance();
    $CI->db->where('project_id', $project_id);
    $CI->db->order_by('milestone_order', 'ASC');
    ...
    $milestoneTasks = array(); // Array to store tasks linked to each milestone
    ...
    foreach ($milestones as $milestone) {
        // Format the start_date and due_date as "M, d, y" (e.g., Jan, 21, 23)
 ...
 * @return mixed
 */
function get_project_startdate($project_id)
{
    $CI = & get_instance();
 ...
 * @return mixed
 */
function get_project_description($project_id)
{
    $CI = & get_instance();
...
/**
 * Get customer name by project ID
 * @param  mixed $project_id project ID
 * @return string|null
 */
function get_customer_name_by_project_id($project_id)
{
    $CI = &get_instance();
    $CI->db->select('tblclients.company');
    $CI->db->join('tblclients', 'tblclients.userid=tblprojects.clientid');
    $CI->db->where('tblprojects.id', $project_id);
    $result = $CI->db->get('tblprojects')->row();
